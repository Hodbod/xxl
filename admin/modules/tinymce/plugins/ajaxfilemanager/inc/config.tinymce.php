...
//error_reporting(E_ALL ^ E_NOTICE);	
	...
	define('CONFIG_SYS_DEFAULT_PATH', '../../../../../upload/'); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', '../../../../../upload/');	//accept relative path only
	...
	define('CONFIG_THICKBOX_MAX_HEIGHT', 430);
		 ...
		 * 		
		 */
  $wroot=dirname(dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))));
  define('CONFIG_WEBSITE_DOCUMENT_ROOT', $wroot);
	...
	define('CONFIG_DEFAULT_VIEW', (CONFIG_SYS_THUMBNAIL_VIEW_ENABLE?'thumbnail':'detail')); //thumnail or detail
	...
	define('CONFIG_LANG_DEFAULT', (CONFIG_QUERY_STRING_ENABLE && !empty($_GET['language']) && file_exists(DIR_LANG . secureFileName($_GET['language'] . '.php'))?secureFileName($_GET['language']):'cs')); //change it to be your language file base name, such en
?>
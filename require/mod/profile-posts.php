    ...
    if(mysql_num_rows($posts) != 0) {
        while($post = mysql_fetch_assoc($posts)) {
            switch($post['type']) {
                case 1:
                case 3:
                    $homelink = _linkRoot($post['home']);
                    $hometitle = mysql_fetch_assoc(mysql_query("SELECT title FROM `"._mysql_prefix."-root` WHERE id=".$post['home']));
                    $hometitle = $hometitle['title'];
                    break;
                case 2:
                    $homelink = _linkArticle($post['home']);
                    $hometitle = mysql_fetch_assoc(mysql_query("SELECT title FROM `"._mysql_prefix."-articles` WHERE id=".$post['home']));
                    $hometitle = $hometitle['title'];
                    break;
                case 5:
                    ...
                    if($post['xhome'] == '-1') {
                        ...
                        $hometitle = mysql_fetch_assoc(mysql_query("SELECT subject FROM `"._mysql_prefix."-posts` WHERE id=".$post['xhome']));

...
/*--- pripava promennych ---*/
$message = "";
$query = mysql_fetch_assoc(mysql_query("SELECT * FROM `"._mysql_prefix."-users` WHERE id="._loginid));
if($query['icq'] == 0) {
    $query['icq'] = "";
}
...
if(_uploadavatar) {
    ...
    if(!@file_exists($avatar_path)) {
        $avatarsrc = _templateImage("system/no-avatar.gif");
    } else {
...
if(isset($_POST['username'])) {
    ...
    if($publicname != $query['publicname'] and $publicname != "") {
        if(mysql_result(mysql_query("SELECT COUNT(id) FROM `"._mysql_prefix."-users` WHERE (publicname='".$publicname."' OR username='".$publicname."') AND id!="._loginid), 0) != 0) {
    ...
    if(!_validateEmail($email)) {
        ...
        if($email != _loginemail) {
            if(mysql_result(mysql_query("SELECT COUNT(id) FROM `"._mysql_prefix."-users` WHERE email='".$email."' AND id!="._loginid), 0) != 0) {
    ...
    if(_uploadavatar) {
        ...
        if(_checkGD("jpg", true)) {
            if(isset($_FILES['avatar'])) {
                ...
                if(!isset($tmp_image_info['extension'])) {
                    $tmp_image_info['extension'] = "";
                }
                if(in_array(mb_strtolower($tmp_image_info['extension']), $__image_ext)) {
                    ...
                    switch(mb_strtolower($tmp_image_info['extension'])) {
                        case "jpg":
                        case "jpeg":
                            $avatar_source = @imagecreatefromjpeg($tmp_image);
                            break;
                        case "png":
                            $avatar_source = @imagecreatefrompng($tmp_image);
                            break;
                        case "gif":
                            $avatar_source = @imagecreatefromgif($tmp_image);
                            break;
                    }
        ...
        if(!_validateURL($avatar)) {
            $avatar = "";
        } else {
            $avatar = _safeStr($avatar);
        }
    }
    ...
    if(_language_allowcustom) {
        ...
        if(!@file_exists(_indexroot."languages/".$language.".php")) {
            $language = "";
        }
    }
    ...
    if(count($errors) == 0) {
        ...
        if(_loginright_administration) {
            mysql_query("UPDATE `"._mysql_prefix."-users` SET wysiwyg=".$wysiwyg." WHERE id="._loginid);
        }
        if(_language_allowcustom) {
            mysql_query("UPDATE `"._mysql_prefix."-users` SET language='".$language."' WHERE id="._loginid);
        }
        if($passwordchange == true) {
            mysql_query("UPDATE `"._mysql_prefix."-users` SET password='".$newpassword[0]."', salt='".$newpassword[1]."' WHERE id="._loginid);
            $_SESSION[_sessionprefix."password"] = $newpassword[0];
        }
        if($usernamechange == true) {
            mysql_query("UPDATE `"._mysql_prefix."-users` SET username='".$username."' WHERE id="._loginid);
        }

...
if($q != "") {
    if(mb_strlen($q) >= 3) {
        ...
        if($art) {
            if(!_loginindicator) {
                $public = " AND ((SELECT public FROM `"._mysql_prefix."-root` WHERE id=`"._mysql_prefix."-articles`.home1)=1 OR (SELECT public FROM `"._mysql_prefix."-root` WHERE id=`"._mysql_prefix."-articles`.home2)=1 OR (SELECT public FROM `"._mysql_prefix."-root` WHERE id=`"._mysql_prefix."-articles`.home3)=1)";
            } else {
                $public = "";
            }
        ...
        if($post) {
            ...
            while($item = mysql_fetch_assoc($query)) {
                ...
                switch($item['type']) {
                    ...
                    case 3:
                    case 5:
                        $homedata = mysql_fetch_assoc(mysql_query("SELECT title,public,visible,var2 FROM `"._mysql_prefix."-root` WHERE id=".$item['home']));
                        break;
                    case 2:
                        $homedata = mysql_fetch_assoc(mysql_query("SELECT title,public,visible FROM `"._mysql_prefix."-articles` WHERE id=".$item['home']));
                        break;
                }

                if(!_publicAccess($homedata['public']) or $homedata['visible'] != 1) {
                    continue;
                }
                ...
                switch($item['type']) {
                    case 1:
                        $homelink = _addGetToLink(_linkRoot($item['home']), "page="._resultPagingGetItemPage(_commentsperpage, "posts", "id>".$item['id']." AND type=1 AND xhome=-1 AND home=".$item['home']))."#posts";
                        break;
                    case 2:
                        $homelink = _addGetToLink(_linkArticle($item['home']), "page="._resultPagingGetItemPage(_commentsperpage, "posts", "id>".$item['id']." AND type=2 AND xhome=-1 AND home=".$item['home']))."#posts";
                        break;
                    case 3:
                        $homelink = _addGetToLink(_linkRoot($item['home']), "page="._resultPagingGetItemPage($homedata['var2'], "posts", "id>".$item['id']." AND type=3 AND xhome=-1 AND home=".$item['home']))."#posts";
                        break;
                    case 5:
                        if($item['xhome'] == -1) {
                            $homelink = "index.php?m=topic&amp;id=".$item['id'];
                        } else {
                            $homelink = _addGetToLink("index.php?m=topic&amp;id=".$item['xhome'], "page="._resultPagingGetItemPage(_commentsperpage, "posts", "id<".$item['id']." AND type=5 AND xhome=".$item['xhome']." AND home=".$item['home']))."#posts";
                        }
                        break;
                }
                ...
                if($item['subject'] == "") {
                    $item['subject'] = $_lang['mod.messages.nosubject'];
                }

...
if(isset($_GET['id'])) {
    ...
    if(mysql_num_rows($query) != 0) {
        ...
        if(_postAccess($query)) {
            ...
            switch($query['type']) {
                case 1:
                    $backlink = _addGetToLink(_linkRoot($query['home']), "page="._resultPagingGetItemPage(_commentsperpage, "posts", "id>".$query['id']." AND type=1 AND xhome=-1 AND home=".$query['home']))."#posts";
                    break;
                case 2:
                    $backlink = _addGetToLink(_linkArticle($query['home']), "page="._resultPagingGetItemPage(_commentsperpage, "posts", "id>".$query['id']." AND type=2 AND xhome=-1 AND home=".$query['home']))."#posts";
                    break;
                case 3:
                    $postsperpage = mysql_fetch_assoc(mysql_query("SELECT var2 FROM `"._mysql_prefix."-root` WHERE id=".$query['home']));
                    $backlink = _addGetToLink(_linkRoot($query['home']), "page="._resultPagingGetItemPage($postsperpage['var2'], "posts", "id>".$query['id']." AND type=3 AND xhome=-1 AND home=".$query['home']))."#posts";
                    break;
                case 4:
                    $nobbcode = true;
                    break;
...
if(isset($_POST['text']) and $continue) {
    ...
    if(!_checkboxLoad("delete")) {
        ...
        if($text != "") {
            if($guest != null) {
                mysql_query("UPDATE `"._mysql_prefix."-posts` SET guest='".$guest."' WHERE id=".$id);
            }
        ...
        if($query['xhome'] == -1) {
            mysql_query("DELETE FROM `"._mysql_prefix."-posts` WHERE xhome=".$id." AND home=".$query['home']." AND type=".$query['type']);
        }
...
if($continue) {
    ...
    if($query['xhome'] == -1 and $query['type'] != 4) {
        $inputs[] = array($_lang[(($query['type'] != 5) ? 'posts.subject' : 'posts.topic')], "<input type='text' name='subject' class='inputsmall' maxlength='22' value='".$query['subject']."' />");
    }

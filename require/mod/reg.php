...
if(isset($_POST['username'])) {
    ...
    } elseif(mysql_result(mysql_query("SELECT COUNT(id) FROM `"._mysql_prefix."-users` WHERE username='".$username."' OR publicname='".$username."'"), 0) != 0) {
    ...
    if(mysql_result(mysql_query("SELECT COUNT(id) FROM `"._mysql_prefix."-users` WHERE email='".$email."'"), 0) != 0) {
    ...
    if(count($errors) == 0) {
        mysql_query("INSERT INTO `"._mysql_prefix."-users` (`group`,levelshift,username,password,salt,logincounter,registertime,activitytime,blocked,massemail,wysiwyg,language,ip,email,avatar,web,skype,msn,jabber,icq,note) VALUES (".$group.",0,'".$username."','".$password[0]."','".$password[1]."',0,".time().",".time().",0,".$massemail.",0,'','"._userip."','".$email."','','','','','',0,'')");
        _iplogUpdate(5);
        $done = true;
    } else {
...
if($done == false) {
    ...
    if(_registration_grouplist) {
        ...
        if(mysql_num_rows($groupselect_items) != 0) {
            $groupselect_content = "";
            while($groupselect_item = mysql_fetch_assoc($groupselect_items)) {
    ...
    $module .= "<p class='bborder'>".$_lang['mod.reg.p']."</p>".$message._formOutput("regform", "index.php?m=reg", array(array($_lang['login.username'], "<input type='text' name='username' class='inputsmall' maxlength='24'"._restorePostValue('username')." />"), array($_lang['login.password'], "<input type='password' name='password' class='inputsmall' />"), array($_lang['login.password']." (".$_lang['global.check'].")", "<input type='password' name='password2' class='inputsmall' />"), array($_lang['global.email'], "<input type='text' name='email' class='inputsmall' "._restorePostValue('email', '@')." />"), array($_lang['mod.settings.massemail'], "<input type='checkbox' name='massemail' value='1' checked='checked' /> ".$_lang['mod.settings.massemail.label']), $groupselect, $captcha, $rules, ), array("username", "email", "password", "password2"), $_lang['mod.reg.submit']);

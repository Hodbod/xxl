...
if(_template_autoheadings == 1) {
    $module .= "<h1>".$_lang['mod.messages']."</h1>\n";
}

//menu
$menu_items = array(1 => $_lang['mod.messages.inbox'], 2 => $_lang['mod.messages.outbox'], 3 => $_lang['mod.messages.new']);
...
switch($a) {
    ...
    case 3:
        ...
        if(isset($_POST['text'])) {
            ...
            $errors = array();
            if($subject == "") {
                $subject = $_lang['mod.messages.nosubject'];
            }
            if($text == "") {
                $errors[] = $_lang['mod.messages.error.notext'];
            }
            ...
            if(mysql_num_rows($receiver) != 0) {
                $receiver = mysql_fetch_assoc($receiver);
                $groupblock = mysql_fetch_assoc(mysql_query("SELECT blocked FROM `"._mysql_prefix."-groups` WHERE id=".$receiver['group']));
                if($receiver['blocked'] == 1 or $groupblock['blocked'] == 1) {
                    $errors[] = $_lang['mod.messages.error.blockedreceiver'];
                }
            ...
            //ulozeni
            if(count($errors) == 0) {
                mysql_query("INSERT INTO `"._mysql_prefix."-messages` (sender,receiver,readed,subject,text,time) VALUES ("._loginid.",".$receiver.",0,'".$subject."','".$text."',".time().")");
        ...
        if(isset($_GET['receiver'])) {
            $get_receiver = _anchorStr($_GET['receiver'], false);
        } else {
            $get_receiver = "";
        }
        ...
        if(isset($_GET['subject'])) {
            $get_subject = ((mb_substr($_GET['subject'], 0, 3) != "Re:") ? "Re: " : '').$_GET['subject'];
        } else {
            $get_subject = "";
        }
        ...
        $module .= "
    <div class='hr'><hr /></div>
    "._jsLimitLength(4096, "newmessage", "text")._formOutput("newmessage", "index.php?m=messages&amp;a=3", array(array($_lang['mod.messages.receiver'], "<input type='text' name='receiver' class='inputmedium' maxlength='24'"._restorePostValue("receiver", $get_receiver)." />"), array($_lang['posts.subject'], "<input type='text' name='subject' class='inputmedium'"._restorePostValue("subject", $get_subject)." maxlength='32' />"), array($_lang['posts.text'], "<textarea class='areamedium' rows='5' cols='33' name='text'>"._restorePostValue("text", null, true)."</textarea>", true)), array("text", "receiver"), null, _getPostformControls("newmessage", "text"));
        break;
    ...
    case 4:
        ...
        if(isset($_GET['id']) and isset($_GET['r'])) {
            ...
            if(mysql_num_rows($message) != 0) {
                ...
                if($message['receiver'] == _loginid and $message['readed'] == 0) {
                    mysql_query("UPDATE `"._mysql_prefix."-messages` SET readed=1 WHERE id=".$id);
                }
                // odkaz na odpoved
                if($return == 1) {
                    $sender_name = mysql_query("SELECT `username` FROM `"._mysql_prefix."-users` WHERE id=".$message['sender']);
                    if(mysql_num_rows($sender_name) != 0) {
                        $sender_name = mysql_fetch_assoc($sender_name);
        ...
        <strong>".$_lang['global.posttime'].":</strong> "._formatTime($message['time']).$answerlink."
        </p>
        <div class='hr'><hr /></div>
        <p>
    ...
    default:
        ...
        if($a == 2) {
            $module .= "<p>".$_lang['mod.messages.outbox.p']."</p>";
        }

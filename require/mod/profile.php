...
if(isset($_GET['id'])) {
    ...
    if(mysql_num_rows($query) != 0) {
        ...
        $groupdata = mysql_fetch_assoc(mysql_query("SELECT title,icon,blocked FROM `"._mysql_prefix."-groups` WHERE id=".$query['group']));
        $form = true;
        ...
        if($query['note'] == "") {
            ...
            $note = "<tr valign='top'><td><strong>".$_lang['global.note']."</strong></td><td><div class='note'>"._parsePost($query['note'])."</div></td></tr>";
        }
        ...
        if(_uploadavatar) {
            ...
            if(!@file_exists($avatar_path)) {
                $query['avatar'] = _templateImage("system/no-avatar.gif");
            } else {
...
if($form == true) {
  ...
  ";
    ...
    if(_loginindicator and _messages and $query['id'] != _loginid and $query['blocked'] == 0 and $groupdata['blocked'] == 0) {
        $module .= "<p><img src='"._templateImage("icons/bubble.png")."' alt='msg' class='icon' /> <a href='index.php?m=messages&amp;a=3&amp;receiver=".$query['username']."'>".$_lang['mod.messages.new']." &gt;</a></p>";
    }
